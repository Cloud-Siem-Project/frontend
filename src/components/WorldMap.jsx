...
function WorldMap({ resources, connections }) {
  ...
  const [center, setCenter] = useState([20, 35]);
          ...
          {beams.map((beam) => {
            ...
            return (
              <Marker key={beam.id} coordinates={[x, y]}>
                <circle r={3 * markerScale} fill="rgba(56, 189, 248, 0.9)" />
                <circle r={6 * markerScale} fill="rgba(56, 189, 248, 0.3)" />
              </Marker>
            );
          })}
